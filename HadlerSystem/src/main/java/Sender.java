...
public class Sender {
    ...
    public void sendVerifierUpdate(String verifierId, String sensorId, Boolean sensorIsWorking, String status) throws StatusRuntimeException {
        ...
        Transaction.Payload payload =
                ransaction.Payload.newBuilder()
                        .setCreatedTime(System.currentTimeMillis())
                        .setCreatorAccountId(verifierId)
                        .setTxCounter(1527606799678L)
                        .addCommands(command)
                        .build();
        Transaction request = newBuilder().setPayload(payload).build();
        Empty response;
    ...
    public void sendSensorUpdate(String sensorId, String parameter, Object value) {
        ...
        Transaction.Payload payload =
                Transaction.Payload.newBuilder()
                        .setCreatedTime(timestamp)
                        .setCreatorAccountId(sensorId)
                        .setTxCounter(1)
                        .addCommands(command)
                        .build();
        Transaction request = Transaction.newBuilder().setPayload(payload).build();
        Empty response;
        try {
            response = blockingStub.torii(request);
